...
class DashboardComparativo {
  ...
  renderMetrics() {
    ...
    if (!comparativo || !comparativo.resumo_geral) return;
    ...
    const totalPilares = comparativo.comparativo_pilares ? comparativo.comparativo_pilares.length : 0;
  ...
  renderPilaresComparativo() {
    ...
    if (!comparativo || !comparativo.comparativo_pilares || !container) return;
  ...
  renderComentariosAnalysis() {
    ...
    if (!container || !comentarios || !comentarios.analise_comentarios_2026) return;
  ...
  setupEvolucaoChart() {
    ...
    if (!ctx || !comparativo || !comparativo.resumo_geral) return;
  ...
  setupPilaresChart() {
    const ctx = document.getElementById('pilaresChart');
    const { comparativo } = this.data;
    
    if (!ctx || !comparativo || !comparativo.comparativo_pilares) return;
    
    const pilares = comparativo.comparativo_pilares;
    
    this.charts.pilares = new Chart(ctx.getContext('2d'), {
      type: 'bar',
      data: {
  ...
  exportExcel() {
    ...
    if (comparativo.comparativo_pilares) {
      csv += '\n\nPilares\n';
  ...
  generateEmbedCode() {
    ...
    const copyButton = modal.querySelector('button');
