...
export const pillarMapping = {
  'Ambiente de Trabalho': { start: 1, end: 3 },
  'Comprometimento Organizacional': { start: 4, end: 7 },
  'Comunicação': { start: 8, end: 10 },
  'Gestão do Capital Humano': { start: 11, end: 26 },
  'Liderança': { start: 27, end: 41 },
  'Trabalho em Equipe': { start: 42, end: 45 }
};

// IDs de perguntas negativas (invertidas)
export const NEGATIVE_QUESTIONS = [20, 23, 45];

// Unidades disponíveis
export const UNITS = ['Eusebio', 'Fortaleza', 'São Luis', 'Juazeiro do Norte'];

// Labels das avaliações
export const RATING_LABELS = ['Discordo muito', 'Discordo', 'Concordo', 'Concordo muito'];
export const RATING_EMOJIS = ['😡', '😟', '😐', '🤩'];

// Retorna o pilar de uma pergunta pelo ID
export function getPillarByQuestionId(qId) {
  for (const [pillar, range] of Object.entries(pillarMapping)) {
    if (qId >= range.start && qId <= range.end) {
      return pillar;
    }
  }
  return '';
...
export const pillars = [...new Set(questions.map(q => q.pillar))];
